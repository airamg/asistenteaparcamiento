...
void srf08_change_i2c_address(unsigned char old_i2c_address, unsigned char new_i2c_address)
{
    /* Start the I2C address changing procedure */
    I2C_START_TX(old_i2c_address);         
...
void start_ping(unsigned char metric_unit)
{
	I2C_START_TX(address);
	i2c_transmit(0);                    
	i2c_transmit(metric_unit);
...
unsigned char check_ping(void)
{
	unsigned char byte;
	
	i2c_start();
	byte=i2c_transmit(address);               
   i2c_stop();
	
	if (byte==I2C_NO_ERROR)
		return (1);
	else
		return (0);
